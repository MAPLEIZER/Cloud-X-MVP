...
export function ConnectionStatus({ className, showText = true, size = 'md' }: ConnectionStatusProps) {
  const { isConnected, isChecking, checkConnection, lastChecked } = useConnection();
  ...
  const getIcon = () => {
    if (isChecking) return <Loader2 className={cn(sizeClasses[size], 'animate-spin')} />;
