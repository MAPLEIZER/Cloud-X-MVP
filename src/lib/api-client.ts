...
class CloudXApiClient {
  ...
  constructor(baseURL: string = 'http://192.168.100.37:5001', timeout: number = 30000) {
  ...
  async getScanStatus(jobId: string): Promise<ScanStatus> {
    return this.request<ScanStatus>(`/api/scan/${jobId}`);
  }

  async stopScan(jobId: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(`/api/scan/${jobId}/stop`, {
      method: 'POST',
    });
  }
