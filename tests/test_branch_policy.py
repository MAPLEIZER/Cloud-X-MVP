...
class BranchPolicyTests(unittest.TestCase):
    ...
    def test_exactly_five_permanent_branches(self):
        self.assertEqual(
            self.policy["permanent_branches"],
    ...
    def test_agent_lane_mapping_is_permanent(self):
        self.assertEqual(
            self.policy["agent_lanes"],
            {"backend": "feature/backend", "frontend": "feature/frontend"},
        )
        self.assertTrue(
            set(self.policy["agent_lanes"].values())
            <= set(self.policy["permanent_branches"])
        )
    ...
    def test_merge_strategy_is_correct_for_every_edge(self):
        ...
        self.assertEqual(
            {edge: cfg["merge_strategy"] for edge, cfg in self.policy["edges"].items()},
            expected,
        )

    def test_unknown_topic_branch_is_denied(self):
        self.assertEqual(
            branch_policy.classify_branch("agent/random-task", self.policy)[0],
            "violation",
        )
    ...
    def test_future_manager_namespaces_are_disabled(self):
        for branch in ("promote/dev-to-staging-x", "salvage/old-work"):
            self.assertEqual(
                branch_policy.classify_branch(branch, self.policy)[0],
                "future-disabled",
            )
    ...
    def test_dependabot_is_enforced_through_dev(self):
        ...
        self.assertTrue(dep["requires_open_pr"])
        self.assertTrue(dep["never_auto_merge"])

    def test_valid_lane_routes(self):
        branch_policy.validate_pr_route(
            "feature/backend",
            "dev",
            "MAPLEIZER",
    ...
    def test_automation_enables_only_bounded_cleanup(self):
        automation = self.policy["automation"]
        self.assertTrue(automation["branch_manager_enabled"])
        self.assertTrue(automation["audit_apply_enabled"])
        ...
        self.assertFalse(self.policy["enforcement"]["destructive_recovery"])

    def test_cleanup_write_permission_is_job_scoped_and_bounded(self):
        workflow = (
            ROOT / ".github" / "workflows" / "branch-enforcement.yml"
        ).read_text(encoding="utf-8")
        self.assertIn("Branch Policy Enforcement", workflow)
        policy_section, cleanup_section = workflow.split("  cleanup:", 1)
        self.assertNotIn("contents: write", policy_section)
        self.assertIn("contents: write", cleanup_section)
        self.assertIn("dependabot/*", cleanup_section)
        self.assertIn(
            "main|staging|dev|feature/backend|feature/frontend", cleanup_section
        )
        self.assertIn("Refusing to delete permanent branch", cleanup_section)
        self.assertIn("-X DELETE", cleanup_section)
        self.assertNotIn("-X DELETE", policy_section)
